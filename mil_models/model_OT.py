...
class OT(nn.Module):
    ...
    def representation(self, x):
        ...
        qq = torch.ones([x.shape[1], self.config.out_size]) / self.config.out_size
        return {'repr': out, 'qq': qq}
    ...
    def predict(self, data_loader, use_cuda=True):
        if self.mode == 'classification':
            output, y, qqs = predict_clf(self, data_loader.dataset, use_cuda=use_cuda)
