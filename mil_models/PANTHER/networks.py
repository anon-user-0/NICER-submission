...
class DirNIWNet(nn.Module):
	...
	def map_em(self, data, mask=None, num_iters=3, tau=1.0, prior=None):
		...
		peak_start = torch.cuda.memory_allocated()
		...
		for emiter in range(num_iters):
			_, qq, _ = mog_eval((pi, mu, Sigma), data)  # E-step: compute log-likelihood
			qq = qq.exp() * mask.unsqueeze(-1)

			pi, mu, Sigma = self.map_m_step(data, weight=qq, tau=tau, prior=prior)  # M-step: update pi, mu, sigma 
		peak_end = torch.cuda.memory_allocated()
		print(f"Peak memory usage during EM: {(peak_end - peak_start) / 1e6} MB")
