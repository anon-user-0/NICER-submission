...
def predict_clf(self, dataset, use_cuda=True, permute=False):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        with torch.no_grad():
            out = self.representation(data)
            repr = out['repr'].detach().cpu()
            qq = out['qq'].detach().cpu()
...
def predict_clf_svGPFA(self, dataset, use_cuda=True, permute=False):
    """
    Create prototype-based slide representation
    ...
    for i in tqdm(range(len(dataset))):
        ...
        out = self.representation(data) # training in this step
        out = out['repr'].data.detach().cpu()
...
def predict_clf_nonparam(self, dataset, use_cuda=True, permute=False):
    """
    Create prototype-based slide representation (non param)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        with torch.no_grad():
            out = self.representation(data, idx=i)
            repr = out['repr'].data.detach().cpu()   # Bxn_protox(2d + 1)
            try:
                qq = out['qq'].data.detach().cpu()
            except:
                qq = out['qq']
            if repr.size(1) > longest: longest = repr.size(1)
    ...
    if X[0].ndim>2:
        ...
        for i in tqdm(range(len(X))):
            B, np, d = X[i].shape
            padded_x = torch.zeros([B, longest, d])
            padded_x[:, :np, :] = X[i]
            X[i] = padded_x   
            
            mask = torch.zeros([B, longest, longest])
            for b in range(B):
                mask[b, :np, :np] = 1.0  # 1 for valid positions, 0 for padded
            
            masks.append(mask)
            M = torch.cat(masks).long()
            # M = None
    else:
        M = None
    # M = None
...
def predict_surv_svGPFA(self, dataset,  use_cuda=True, permute=False):
    """
    Create prototype-based slide representation
    """
...
def predict_surv_nonparam(self, dataset,  use_cuda=True, permute=False):
    """
    Create prototype-based slide representation
    """
    ...
    for i in tqdm(range(len(dataset))):
        ...
        with torch.no_grad():
            batch_out = self.representation(data)
            batch_out = batch_out['repr'].data.cpu()
            if batch_out.size(1) > longest: longest = batch_out.size(1)
    ...
    if output[0].ndim>2:
        ...
        for i in tqdm(range(len(output))):
            B, np, d = output[i].shape
            padded_x = torch.zeros([B, longest, d])
            padded_x[:, :np, :] = output[i]
            output[i] = padded_x   
