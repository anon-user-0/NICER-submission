...
def compress(data_loader, n_proto, n_components, n_init=5, feature_dim=1024, n_proto_patches=50000, mode='kmeans', use_cuda=False):
    """
    ...
    print(f"{n_patches_per_batch} samples per batch (image)")
    ...
    if mode == 'pca':
        ...
        print(f"\n\tNum of clusters {n_proto}, num of components {n_components}")
        pca = PCA(n_components=n_components)
        compressed = pca.fit(patches[:n_patches].cpu())
        V = pca.components_.T
    ...
    return compressed, V, mean_vector
