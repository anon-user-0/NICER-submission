...
PROTO_MODELS = ['PANTHER', 'OT', 'H2T', 'ProtoCount', 
                'DeepSets', 'NonParam_Local_Groups', 'InfiniteGPFA', 'KMeans',
                'DPMeans', 'AdaptiveGMM']
...
def build_datasets(csv_splits, model_type, batch_size=1, num_workers=2,
    ...
    for k in csv_splits.keys(): # ['train', 'val', 'test']
        ...
        # data_sampler = build_sampler(dataset, sampler_type='sequential')
...
##
# model / loss fn args ###
parser.add_argument('--model_type', type=str, choices=['H2T', 'ABMIL', 'TransMIL', 'DSMIL', 'SumMIL', 'DeepSets',
                                                       'OT', 'PANTHER', 'ProtoCount', 'DeepAttnMIL', 
                                                       'ILRA', 'NonParam', 'NonParam_Local_Groups',
                                                       'InfiniteGPFA', 'KMeans', 'DPMeans',
                                                       'AdaptiveGMM'],
                    default='ABMIL',
                    help='type of model')
...
parser.add_argument('--similarity_lr', type=float, default=0.1)
parser.add_argument('--top_k', type=int, default=3)

# Compression related
parser.add_argument('--load_compression', action='store_true', default=False)
parser.add_argument('--compression_path', type=str, default='.')
...
parser.add_argument('--sample_col', type=str, required=True,
                    help='sample col name to load data from csv')
parser.add_argument('--label_col', type=str, required=True,
                    help='label col name to load data from csv')
parser.add_argument('--dd_test', type=bool, default=False,
                    help="use dataset distillation testing, i.e., train on distilled set and test on original set")
